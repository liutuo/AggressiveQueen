# Aggressive Queens (AQ) Solver

This repository contains solvers for the "Aggressive Queens" problem, a generalization of the classic N-Queens problem. In this version, the goal is to find configurations on an $n \times n$ board where every queen experiences exactly $k$ attacks from other queens. The classic N-Queens problem is a special case where $k=0$.

The solvers aim to find the maximum possible number of queens for a given $k$ and $w$ (toroidal board flag).

## Files

- `find_aq_seq.c`: Optimized sequential solver.
- `find_aq_par.c`: Optimized parallel solver using MPI.
...
**Compilation:**
```bash
mpicc -o find_aq_par find_aq_par.c -lm
```

**Running:**
```bash
mpirun -np <num_procs> ./find_aq_par n k l w
```
...
- **Correct MPI Aggregation:** The parallel solver correctly synchronizes the global maximum queen count and aggregates/deduplicate solutions on the root process.
