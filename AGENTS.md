# Developer Guide for AQ Solvers

## Problem Domain
The 'Almost Queens' algorithm finds board configurations where each queen has exactly `k` attacks. The board is represented as a 1D integer array of size `n*n`.
...
## MPI Synchronization (`findAQ_par.c`)
...
## Testing
Always run `./test_aq.sh` before submitting changes to ensure no regressions in the sequential core logic.
